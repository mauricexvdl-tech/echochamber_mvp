...
pub struct TopologyCache {
    ...
    /// Base seed for injector placement (used by SeedAdaptive).
    pub injector_base_seed: u64,
    /// Total number of injectors to place.
...
impl TopologyCache {
    ...
    pub fn new(config: &Config) -> Self {
        ...
        let (injectors, injector_desc) = match config.injector_layout {
            ...
            InjectorLayout::SeedAdaptive => {
                // Don't pre-compute - will generate per simulation seed
                let desc = format!(
                    "SeedAdaptive(M={}, base=0x{:X})",
                    total_injectors, injector_seed
                );
                (Vec::new(), desc)
            }
    ...
    pub fn get_injectors_for_seed(&self, sim_seed: u64) -> Vec<usize> {
        match self.layout {
            ...
            InjectorLayout::Random => {
                // Empty - will be generated by Causes::new
                Vec::new()
            }
        }
    }
    ...
    fn build_chamber_and_causes(
        ...
        // Build causes with appropriate injector layout
        let injectors = self.get_injectors_for_seed(sim_seed);
        let causes = if injectors.is_empty() {
...
pub fn run_single_seed_full(
    ...
    // Phase 2.2c: Pass seed for SeedAdaptive injector placement
    let (mut chamber, causes) = if let Some(cache) = topology_cache {
        cache.build_chamber_and_causes(config, &mut rng, seed)
    } else {
